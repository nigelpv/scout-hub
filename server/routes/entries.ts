...
router.get('/', async (req, res) => {
    try {
        ...
        const mappedEntries = entries.map((row: any) => ({
            ...
            startingPosition: (row.starting_position || 'hub') as 'left_trench' | 'left_bump' | 'hub' | 'right_trench' | 'right_bump',
            ...
            disabledOrShutDown: row.disabled_or_shut_down || false,
            notes: row.notes || '',
        }));
    ...
    try {
        ...
        const { error: insertError } = await supabase
            ...
            .insert({
                ...
                defense_type: JSON.stringify(entry.defenseType || []),
                defense_location: JSON.stringify(entry.defenseLocation || []),
                defense_effectiveness: entry.defenseRating || 0,
                defense_played: Array.isArray(entry.defenseType) && entry.defenseType.length > 0,
                shooting_range: JSON.stringify(entry.shootingRange || []),
                ...
                disabled_or_shut_down: entry.disabledOrShutDown || false,
                notes: entry.notes || ''
            });
